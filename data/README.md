...
## 洪水ハザードによるルート評価（Phase5A / 5A.1で使用）
...
### 【重要】この評価方法の限界（卒論の限界節に記載すべき事項）

1. **区間近似による過大評価の傾向**：サンプル区間は「両端のいずれかでハザードが検出されたら区間全体をハザードありとみなす」という単純な規則を採用している。2026-09-06のサンプリング間隔比較実験（`scripts/research-data/route-sampling-experiment-results.csv`）では、同一ルートで間隔を20m→100mと粗くするほど、洪水区域通過割合がやや高く算出される傾向が確認された（例：ある地点のルートAで20m間隔86.4%→100m間隔99.3%）。間隔が粗いほど過大評価に傾く可能性がある。
2. **サンプル点間の見落とし**：サンプル地点の間に、サンプリングされなかった狭い浸水域（または逆に狭い安全な隙間）が存在する場合、評価に反映されない。実験では観測された最大浸水深ランクは間隔を変えても変化しなかった（同一ルート内では常に同じランクを検出）が、これは今回の5地点10ルートのデータでの結果であり、一般的にランクが変わらないことを保証するものではない。
3. **評価カバー率が0%になりうる**：ハザードタイルは河川の浸水想定調査が行われている範囲にのみ存在し、範囲外では404（データなし）が返る。実験では大阪市港区周辺・天王寺区周辺のルートで**全サンプル地点が「評価不可」となり、評価カバー率0%**となるケースが実際に確認された（タイルサーバーに直接アクセスして404であることを確認済みで、実装のバグではない）。この場合、洪水区域通過割合という指標自体が「評価不能」であり、算出された数値（0%や特定の割合）を信頼できる評価として扱ってはならない。
4. **openrouteserviceのルート形状（頂点間隔）に依存**：サンプリングは頂点を線形補間して行っており、実際の道路の曲線形状を完全には反映しない。

### サンプリング間隔比較実験の記録

- **実行日**: 2026-09-06
- **スクリプト**: `scripts/route-sampling-experiment.mjs`
- **結果ファイル**: `scripts/research-data/route-sampling-experiment-results.json` / `.csv`
- **条件**: 大阪市内5地点（都心河川近く／湾岸低地／内陸高台寄り／東部低地／南部）、各地点で取得できた徒歩ルート（1〜3件）×サンプリング間隔20/30/50/100mの全組み合わせ、計40件
- **採用間隔**: 今回の実験では確定していない（人間の判断待ち。詳細は報告参照）
