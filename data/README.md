...
- **取得元URL（データセットの説明ページ）**: https://www.city.osaka.lg.jp/toshikeikaku/page/0000250227.html
- **実際のCSVダウンロードURL**: https://www.mapnavi.city.osaka.lg.jp/osakacity/osakacity/opendatafile/map_2/CSV/opendata_2015.csv
- **取得日**: 2026-09-26
- **ライセンス**: CC BY（大阪市の該当ページに明記。第三者への提供・改変時は出典表示が必要）
- **更新頻度（大阪市側）**: マップナビおおさかの掲載情報が更新された翌日に自動作成（＝日次相当。ただしこのプロジェクトでは取得日時点のスナップショットを`data/raw/`に固定して使用する）
- **生データ**: `raw/osaka-city-opendata-shelters.csv`（1528件、UTF-8 BOM付き）
- **含まれる項目**: 場所の名前, カテゴリ, 区名, 所在地, TEL, 避難可能時間, 災害対策基本法に基づく指定, 避難場所の対象とする災害, 経度, 緯度, 分類
  - 「受入可能人数」に相当する項目は存在しない（規模分類を実装しない理由）。
  - 「避難場所の対象とする災害」は地震・津波が中心で、洪水・内水氾濫等の既存hazards判定には使用していない（GSIデータのhazardsフィールドをそのまま維持）。
- **名寄せ方法**: GSIデータ（指定緊急避難場所・指定避難所）と、座標距離（120m以内）かつ正規化した施設名の一致/包含を両方満たす場合のみ紐付ける。候補が複数あっても正規化名が全て同一なら（同一施設の重複登録として）安全に全件へ補完し、正規化名が食い違う場合は「あいまい」として補完しない（安全側優先。誤った電話番号等の付与を避ける）。
- **名寄せ結果（2026-09-26実行時点）**: 大阪市データ1528件中、マッチ成功1471件・未マッチ37件・あいまい20件（詳細は`node scripts/build-shelters.mjs`実行時のコンソール出力を参照。ログはコード内に埋め込まず、実行時にのみ出力する）
